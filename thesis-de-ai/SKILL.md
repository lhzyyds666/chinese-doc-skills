...
2. **unpack docx**：`python C:/Users/User/.Codex/skills/docx/scripts/office/unpack.py <input.docx> _ws/unpacked/`
3. **改写**：在 `_ws/unpacked/word/document.xml` 里只改 `<w:t>` 内文本，不动 XML 结构。含引用 hyperlink 的段落是多 run 拼接，需按 run 边界拆分；插入新引用见下方
4. **repack**：`python C:/Users/User/.Codex/skills/docx/scripts/office/pack.py _ws/unpacked/ <output.docx> --original <input.docx> --validate false`（中文文件名导致 GBK 验证误报，但实际文件正常生成）
5. **验证**：
